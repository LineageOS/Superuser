...
static int setup_daemon_child(int pid, int argc, char** argv) {
    ...
    LOGD("child infile: %s", infile);
    LOGD("child outfile: %s", outfile);
...
static int connect_daemon(int argc, char *argv[]) {
    ...
    LOGD("client infile: %s", infile);
    LOGD("client outfile: %s", outfile);
    unlink(infile);
    unlink(outfile);
    ...
    files[1] = infd;
    // pthread_create(&input_reader, NULL, pump_thread, files);
